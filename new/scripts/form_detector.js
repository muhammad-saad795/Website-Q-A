...
/* ==========================================================================
   FORM INTERACTION & SUBMISSION ENGINE
   ========================================================================== */

/**
 * Main orchestrator for form QA
 */
async function processForms() {
    const forms = detectForms();
    if (forms.length === 0) return { success: true, forms: [], message: "No forms detected" };

    // Load test cases
    const testCases = window.__FORM_TEST_CASES__ || { fallback: "test" };

    // Target the first high-scoring form
    const targetForm = forms[0];
    const container = document.querySelector(targetForm.containerSelector);

    if (!container) return { success: false, error: "Target container not found", forms };

    const report = {
        form_id: targetForm.id,
        form_selector: targetForm.containerSelector,
        fields: [],
        submission: {
            method: null,
    ...
    try {
        // 1. Fill the form
        report.fields = fillForm(container, testCases);

        // 2. Submit the form
        const submissionResult = await submitForm(container, targetForm.submitSelectors);
        report.submission = submissionResult;

        // 3. Settle
        await new Promise(r => setTimeout(r, 1500));

        report.navigation.final_url = window.location.href;
        report.navigation.url_changed = report.navigation.final_url !== report.navigation.original_url;

        // 4. Analysis
        if (!report.navigation.url_changed) {
            const errors = detectValidationErrors(targetForm.containerSelector);
            if (errors.length > 0) {
                report.submission.success = false;
                report.submission.message = "Validation errors: " + errors.join("; ");
            } else {
                report.submission.success = true;
                report.submission.message = "No navigation, but no visual errors detected.";
            }
        } else {
            report.submission.success = true;
            report.submission.message = "Form submitted and navigated.";
        }

    } catch (err) {
        report.submission.success = false;
        report.submission.message = "Error: " + err.message;
    }

    return { success: true, report: report, all_forms: forms };
}

/**
 * Fills fields in a container
 */
function fillForm(container, testCases) {
    ...
    fields.forEach(el => {
        ...
        let valueToSet = testCases.fallback || "test";

        for (const [key, val] of Object.entries(testCases)) {
            if (name.includes(key) || el.placeholder?.toLowerCase().includes(key)) {
                valueToSet = val;
                break;
            }
        }
        ...
        try {
            if (el.tagName === 'SELECT') {
                if (el.options.length > 0) {
                    el.selectedIndex = (typeof valueToSet === 'number' && valueToSet < el.options.length) ? valueToSet : 1;
                    el.dispatchEvent(new Event('change', { bubbles: true }));
                }
            } else if (el.type === 'checkbox' || el.type === 'radio') {
                el.checked = !!valueToSet;
                el.dispatchEvent(new Event('change', { bubbles: true }));
            } else {
                el.value = valueToSet;
            ...
            filledData.push({ name: el.name || el.id || "unnamed", type: type, value: el.value || el.checked });
        } catch (e) { }
    });
    return filledData;
}
 ...
 * Submission logic
 */
async function submitForm(container, submitSelectors) {
    if (submitSelectors && submitSelectors.length > 0) {
        for (const selector of submitSelectors) {
            const btn = document.querySelector(selector);
            if (btn && isVisible(btn)) {
                btn.scrollIntoView({ behavior: 'auto', block: 'center' });
                btn.click();
                return { method: 'button_click', button_selector: selector, success: true };
            }
        }
    }
    // Fallback: Enter key
    ...
    if (inputs.length > 0) {
        ...
        return { method: 'enter_keypress', success: true };
    }
    return { success: false, message: "No submit element found" };
}
